...
export default function Overview({ eventId }: { eventId: string }) {
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Quick Actions */}
            <div>
                <h3 className="text-sm font-bold text-text mb-4">Ações Rápidas</h3>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <QuickActions icon={<ShareIcon className="w-4 h-4" />} title="Compartilhar Evento" description="Redes sociais e link" />
                    <QuickActions icon={<ArrowDownTrayIcon className="w-4 h-4" />} title="Baixar Relatório" description="PDF ou Excel" />
                    <QuickActions icon={<BellIcon className="w-4 h-4" />} title="Notificar Participantes" description="Envio de e-mail" />
                </div>
            </div>

            {/* Engagement Stats */}
            <div>
                <h3 className="text-sm font-bold text-text mb-4">Engajamento (Simulado)</h3>
            ...
            <div className="text-sm border-t border-borderColor p-2">
                <h3 className="font-bold text-text mb-4">Atividade Recente (Simulado)</h3>
                        ...
                        <div key={i} className="flex items-center justify-between py-3 border-b border-borderColor last:border-0">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="text-text-muted">João Silva comprou 2x Pista Premium</div>
                                </div>
                            </div>
                            <div className="text-text-muted">Há 2 horas</div>
                        </div>
                    ))}
                </div>
            </div>
...
function QuickActions({ icon, title, description }: QuickAction) {
    return (
        <button className="border border-borderColor text-xs rounded p-2 hover:bg-white/5 transition-all hover:border-primary group text-left">
