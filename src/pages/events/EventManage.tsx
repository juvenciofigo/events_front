...
export default function EventManage() {
    ...
    return (
        <div className="min-h-screen bg-background text-text font-sans selection:bg-primary selection:text-white">
            ...
            {/* Content Area */}
            <div className="px-3 bg-surface mt-4  pb-5">
                {/* Tabs */}
                <div className="w-full flex justify-around overflow-x-auto no-scrollbar mb-5 border-b border-borderColor">
                    {TABS.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setSearchParams({ tab: tab.id })}
                            className={`flex items-center text-sm font-bold px-6 justify-center p-2 border-b-2 transition-colors whitespace-nowrap ${activeTab === tab.id
                ...
                {activeTab === "settings" && (<Settings event={eventData} />)}
